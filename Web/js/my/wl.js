	...
	var searchCtrl = function($scope) {
		...
		$scope.changeEnv = function(onlineOrOff) {
			...
			if (onlineOrOff == 0) {
				...
				$scope.databaseUrl = "10.66.0.99/root/123456";
			}
		...
		$scope.getUserList = function(){
			if($scope.searchInfo.userName != "" || $scope.searchInfo.compCode != "" || $scope.searchInfo.cardNo != "" || $scope.searchInfo.compName != ""){
				...
				if(localStorage){
					 ...
					 var info = Util.getNowFormatDate() + "查询老平台:" + $scope.searchInfo.userName + "|" + $scope.searchInfo.compCode + "|" + 
		...
		$scope.savePhoneNo = function(index){
			if($scope.lstUserVO[index].phoneNo == $scope.oldPhoneNo){
					...
					$scope.searchInfo.compCode = $scope.lstUserVO[index].compCode;
					$scope.ajaxSavePhoneNo();
					//加入本地历史记录
					if(localStorage){
						 var history = localStorage.getItem("history");
						 var info = "<span style='color:blue'>" + Util.getNowFormatDate() + '修改老平台:' + $scope.searchInfo.userName + "|" + $scope.searchInfo.compCode + "|" + 
						 $scope.lstUserVO[index].cardNo + "|" + $scope.lstUserVO[index].compName + "的手机号:" + $scope.oldPhoneNo + "为:" + $scope.searchInfo.phoneNo + ";</span></br>";
						 //将信息加入到本地存储
					     localStorage.setItem("history", (!history ? "" : history) + info);
					}
				}else{
